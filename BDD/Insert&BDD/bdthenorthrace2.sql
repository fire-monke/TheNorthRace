...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addClassement` (IN `teamId` INT, IN `year` INT, IN `points` INT, IN `teamPlace` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddEcurie` (IN `nomEcu` VARCHAR(50), IN `couleurEcu` VARCHAR(20), IN `dateCreation` YEAR, IN `localisation` VARCHAR(50), IN `nbTitresConstructeur` INT, IN `nbCoursesDisputees` INT, IN `nbVictoires` INT, IN `nbPoduims` INT, IN `directeur` VARCHAR(80), OUT `idEcurie` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddPilote` (IN `nomPilote` VARCHAR(50), IN `prenomPilote` VARCHAR(50), IN `paysPilote` VARCHAR(50), IN `dateNaissance` DATE, OUT `idPilote` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddRaceYear` (IN `pilotId` INT, IN `yearRace` INT, IN `teamId` INT, IN `points` INT, IN `pilotPlace` INT, IN `pilotNumber` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteClassement` (IN `teamId` INT, IN `year` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteRaceYear` (IN `pilotId` INT, IN `teamID` INT, IN `yearRace` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getClassementByTeam` (IN `teamId` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getClassementByYear` (IN `year` INT)   BEGIN
    SELECT idEcu, annee, nbPointEcu, placeEcu
    FROM classement 
    WHERE annee = year
	ORDER BY placeEcu;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getClassementByYearAndTeam` (IN `year` INT, IN `idEcu` INT)   BEGIN
    SELECT c.idEcu, c.annee, c.nbPointEcu, c.placeEcu, e.nom AS nomEcurie
    FROM Classement c
    INNER JOIN Ecurie e ON c.idEcu = e.id
    WHERE c.annee = year AND c.idEcu = idEcu;
END$$

DROP PROCEDURE IF EXISTS `getCoursesDetailsByPilot`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getCoursesDetailsByPilot` (IN `idPil` INT)   BEGIN
    SELECT c.idPil,c.annee,c.idEcu,c.nbPointPil,c.placePil,c.numPil, p.nom AS nomPilote, p.prenom AS prenomPilote, e.nom AS nomEcurie
    FROM CoursesAnnee c
    INNER JOIN Pilote p ON c.idPil = p.id
    INNER JOIN Ecurie e ON c.idEcu = e.id
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getCoursesDetailsByPilotAndYear` (IN `idPil` INT, IN `year` INT)   BEGIN
    SELECT c.idPil,c.annee,c.idEcu,c.nbPointPil,c.placePil,c.numPil, p.nom AS nomPilote, p.prenom AS prenomPilote, e.nom AS nomEcurie
    FROM CoursesAnnee c
    INNER JOIN Pilote p ON c.idPil = p.id
    INNER JOIN Ecurie e ON c.idEcu = e.id
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getCoursesDetailsByPilotTeamAndYear` (IN `idPil` INT, IN `idEcu` INT, IN `year` INT)   BEGIN
    SELECT c.idPil,c.annee,c.idEcu,c.nbPointPil,c.placePil,c.numPil, p.nom AS nomPilote, p.prenom AS prenomPilote, e.nom AS nomEcurie
    FROM CoursesAnnee c
    INNER JOIN Pilote p ON c.idPil = p.id
    INNER JOIN Ecurie e ON c.idEcu = e.id
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getCoursesDetailsByTeam` (IN `idEcu` INT)   BEGIN
    SELECT c.idPil,c.annee,c.idEcu,c.nbPointPil,c.placePil,c.numPil, p.nom AS nomPilote, p.prenom AS prenomPilote, e.nom AS nomEcurie
    FROM CoursesAnnee c
    INNER JOIN Pilote p ON c.idPil = p.id
    INNER JOIN Ecurie e ON c.idEcu = e.id
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getCoursesDetailsByTeamAndYear` (IN `idEcu` INT, IN `year` INT)   BEGIN
    SELECT c.idPil,c.annee,c.idEcu,c.nbPointPil,c.placePil,c.numPil, p.nom AS nomPilote, p.prenom AS prenomPilote, e.nom AS nomEcurie
    FROM CoursesAnnee c
    INNER JOIN Pilote p ON c.idPil = p.id
    INNER JOIN Ecurie e ON c.idEcu = e.id
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getPodiumByYear` (IN `year` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateClassement` (IN `points` INT, IN `teamPlace` INT, IN `teamId` INT, IN `year` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateEcurieColor` (IN `idEcu` INT(3), IN `new_CouleurEcu` VARCHAR(40), IN `new_dateCreation` YEAR, IN `new_localisation` VARCHAR(50), IN `new_nbTitresConstructeur` INT, IN `new_nbCoursesDisputees` INT, IN `new_nbVictoires` INT, IN `new_nbPoduims` INT, IN `new_directeur` VARCHAR(50))   BEGIN
    UPDATE Ecurie
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateRaceForYear` (IN `pilotId` INT, IN `yearRace` INT, IN `teamId` INT, IN `newPoints` INT, IN `newPilotPlace` INT, IN `newPilotNumber` INT)   BEGIN
    UPDATE CoursesAnnee 
